...
data_all <- read_csv(here("data", "HAS_demo_all.csv")) %>% 
  clean_names() %>% 
  select(subject_id, scan_date, sex, reproductive_status, age_scan_years,
        bmi, self_ses, education_years, verbal_iq,
        
        #Women's basic repro health
        straw_10_stages, age_at_menarche_years, time_since_last_cycle_years,
        menopausal_status,
...
```{r table-demo-all}
#Repo status
tab_age_repo_all <- data_all_repo %>% group_by(reproductive_status) %>% 
    summarise(count = n(),
            mean_age = mean(age_scan_years, na.rm=TRUE),
            sd_age = sd(age_scan_years,na.rm=TRUE),
            min_age = min(age_scan_years,na.rm=TRUE),
            max_age = max(age_scan_years, na.rm=TRUE))
tab_age_repo_all <- tab_age_repo_all %>%  rename("Group" = "reproductive_status")

#Sex
tab_age_sex_all <- data_all %>% group_by(sex) %>% 
  summarise(count = n(),
            mean_age = mean(age_scan_years, na.rm=TRUE),
            sd_age = sd(age_scan_years, na.rm=TRUE),
            min_age = min(age_scan_years, na.rm=TRUE),
            max_age = max(age_scan_years, na.rm=TRUE))
tab_age_sex_all <- tab_age_sex_all %>%  rename("Group" = "sex")

#Total
tab_age_total_all <-  data_all %>% 
  summarise(count = n(),
            mean_age = mean(age_scan_years, na.rm=TRUE),
            sd_age = sd(age_scan_years, na.rm=TRUE),
            min_age = min(age_scan_years, na.rm=TRUE),
            max_age = max(age_scan_years, na.rm=TRUE))
tab_age_total_all <- data.frame(Group = "Total", tab_age_total_all)

#Stack tables together
tab_age_all <- rbind(tab_age_repo_all, tab_age_sex_all, tab_age_total_all)

#Present as kable
tab_age_all %>% 
  kable(col.names = c("Group", "Count", "Mean Age (years)", 
                      "Standard Deviation of Age (years)",
                      "Minimum Age (years)", "Maximum Age (years)"),
        
        #Add caption
        caption = "Age characteristics of ALL HAS sample.",
...
```{r table-demo-clean}
#Repo status
tab_age_repo <- data_sub_repo %>% group_by(reproductive_status) %>% 
    summarise(count = n(),
            mean_age = mean(age_scan_years),
            sd_age = sd(age_scan_years),
            min_age = min(age_scan_years),
            max_age = max(age_scan_years))
tab_age_repo <- tab_age_repo %>%  rename("Group" = "reproductive_status")

#Sex
tab_age_sex <- data_sub %>% group_by(sex) %>% 
  summarise(count = n(),
            mean_age = mean(age_scan_years),
            sd_age = sd(age_scan_years),
            min_age = min(age_scan_years),
            max_age = max(age_scan_years))
tab_age_sex <- tab_age_sex %>%  rename("Group" = "sex")

#Total
tab_age_total <-  data_sub %>% 
  summarise(count = n(),
            mean_age = mean(age_scan_years),
            sd_age = sd(age_scan_years),
            min_age = min(age_scan_years),
            max_age = max(age_scan_years))
tab_age_total <- data.frame(Group = "Total", tab_age_total)

#Stack tables together
tab_age_sub <- rbind(tab_age_repo, tab_age_sex, tab_age_total)
...
tab_age_sub %>% 
  kable(col.names = c("Group", "Count", "Mean Age (years)", 
        ...
        #Add caption
        caption = "Age characteristics of cleaned HAS sample.",
...
Please refer to hand out for means and standard deviations of the basic demographic and neuropsychological batteries. Figures will only include basic demographic and neuropsychiatric batteries.
